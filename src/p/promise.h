...
namespace promise_hpp::detail
{
  ...
  void construct_in_place(T &ref, Args &&...args) noexcept(std::is_nothrow_constructible_v<T, Args...>)
  {
  ...
  public:
    ...
    storage &operator=(T &&value) noexcept(std::is_nothrow_move_constructible_v<T>)
    {
      assert(!initialized_);
    ...
    storage &operator=(const T &value) noexcept(std::is_nothrow_copy_constructible_v<T>)
    {
      assert(!initialized_);
...
namespace promise_hpp
{
  ...
  public:
    ...
    template <typename ResolveF, typename ResolveR = std::invoke_result_t<ResolveF, T>>
    std::enable_if_t<
    ...
    then(ResolveF &&on_resolve)
    {
      ...
      then([n = next,
            f = std::forward<ResolveF>(on_resolve)](auto &&v) mutable {
        auto np = std::invoke(
    ...
    template <typename ResolveF, typename RejectF, typename ResolveR = std::invoke_result_t<ResolveF, T>>
    std::enable_if_t<
        !is_promise_v<ResolveR>,
        promise<ResolveR>>
    ...
    promise<T> finally(FinallyF &&on_finally)
    {
      return then([f = on_finally](auto &&v) {
                ...
                return std::forward<decltype(v)>(v); }, [f = on_finally](std::exception_ptr e) -> T {
                std::invoke(std::move(f));
  ...
  private:
    ...
    public:
      ...
      {
        auto reject_h = [n = next,
                         f = std::forward<RejectF>(on_reject),
                         has_reject](std::exception_ptr e) mutable {
          if (has_reject)
          {
            try
            {
    ...
    private:
      ...
      void add_handlers_(ResolveF &&resolve, RejectF &&reject)
      {
        ...
        {
          handlers_.push_back({std::forward<ResolveF>(resolve),
                               std::forward<RejectF>(reject)});
        }
      }
...
namespace promise_hpp
{
  ...
  public:
    ...
    template <typename ResolveF, typename ResolveR = std::invoke_result_t<ResolveF>>
    std::enable_if_t<
        is_promise_v<ResolveR>,
        promise<typename ResolveR::value_type>>
    then(ResolveF &&on_resolve)
    {
      ...
      then([n = next,
            f = std::forward<ResolveF>(on_resolve)]() mutable {
        auto np = std::invoke(
            std::forward<decltype(f)>(f));
        std::move(np).then([n](auto &&...nvs) mutable {
                       n.resolve(std::forward<decltype(nvs)>(nvs)...);
    ...
    template <typename ResolveF, typename RejectF, typename ResolveR = std::invoke_result_t<ResolveF>>
    std::enable_if_t<
        !is_promise_v<ResolveR>,
        promise<ResolveR>>
    then(ResolveF &&on_resolve, RejectF &&on_reject)
    ...
    {
      return then([f = on_finally]() { std::invoke(std::move(f)); }, [f = on_finally](std::exception_ptr e) {
                std::invoke(std::move(f));
                std::rethrow_exception(e); });
    }
...
namespace promise_hpp
{
  ...
  template <typename Iter, typename SubPromise = typename std::iterator_traits<Iter>::value_type, typename SubPromiseResult = typename SubPromise::value_type, typename ResultPromiseValueType = std::vector<SubPromiseResult>>
  ...
  make_all_promise(Iter begin, Iter end)
  {
    ...
    struct context_t
    {
      ...
      context_t(std::size_t count)
          : success_counter(count), results(count) {}
    };
  ...
  template <typename Iter, typename SubPromise = typename std::iterator_traits<Iter>::value_type, typename SubPromiseResult = typename SubPromise::value_type, typename ResultPromiseValueType = SubPromiseResult>
  promise<ResultPromiseValueType>
  make_any_promise(Iter begin, Iter end)
  {
    ...
    struct context_t
    {
      ...
      context_t(std::size_t count)
          : failure_counter(count), exceptions(count) {}
    };
  ...
  namespace impl
  {
    ...
    {
      return std::get<I>(tuple).then([context,
