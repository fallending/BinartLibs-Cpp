...
inline bool encode(const bool &obj_val, allocator_t &alloc,
...
template <typename T>
bool encode_field(const T &field, const char *field_name, allocator_t &alloc,
                  rapidjson::Value &json_val);
template <typename T>
bool decode_field(const rapidjson::Value &json_val, const char *field_name,
                  T &field, bool &field_in_json);
...
bool encode(const std::map<std::string, T> &obj_val, allocator_t &alloc,
    ...
    for (it = obj_val.begin(); it != obj_val.end(); ++it) {
        ...
        json_val.AddMember(rapidjson::StringRef(it->first.c_str()), val.Move(),
                           alloc);
    }
    return true;
}
